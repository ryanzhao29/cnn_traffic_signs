...
class def_leNet():
    ...
    def get_detection_model(self):
        ...
        weights_fc1 = new_weights(shape = [num_fueatures, self.num_fc_layer1])
        bias_fc1 = new_bias(self.num_fc_layer1)
        #transform this weight and bias for conv layer
        weights_conv_fc1 = tf.reshape(weights_fc1, [int(image_size/4), int(image_size/4), self.num_filter_conv_layer2, self.num_fc_layer1])
        layer_conv_fc1 = tf.nn.conv2d(conv2, weights_conv_fc1, [1,1,1,1], 'VALID') + bias_fc1
        layer_conv_fc1 = tf.nn.relu(layer_conv_fc1)
        #repeat this for fc2
        weights_fc2 = new_weights(shape = [self.num_fc_layer1, self.num_fc_layer2])
        bias_fc2 = new_bias(self.num_fc_layer2)
        #transform this weight and bias for conv layer
        weights_conv_fc2 = tf.reshape(weights_fc2, [1, 1 , self.num_fc_layer1, self.num_fc_layer2])
        layer_conv_fc2 = tf.nn.conv2d(layer_conv_fc1, weights_conv_fc2, [1,1,1,1], 'VALID') + bias_fc2
        layer_conv_fc2 = tf.nn.relu(layer_conv_fc2)
        #repeat this for fc3
        weights_fc3 = new_weights(shape = [self.num_fc_layer2, self.num_fc_layer3])
        bias_fc3 = new_bias(self.num_fc_layer3)
        #transform this weight and bias for conv layer
        weights_conv_fc3 = tf.reshape(weights_fc3, [1, 1 , self.num_fc_layer2, self.num_fc_layer3])
        layer_conv_fc3 = tf.nn.conv2d(layer_conv_fc2, weights_conv_fc3, [1,1,1,1], 'VALID') + bias_fc3
