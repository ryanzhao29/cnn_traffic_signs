...
class def_leNet():
    ...
    def get_training_model(self, enable_dropout = True, feature_extraction_phase = feature_extraction_status.bottleneck):
        ...
        if feature_extraction_phase == feature_extraction_status.bottleneck: #only the bottleneck training uses the fc layer of the original lenet as the output
            ...
            fc2 = tf.stop_gradient(fc2) #stop gradient so only optimze bias_fc3_usa and weights_fc3_usa
            fc3, self.weights_fc3_usa, self.bias_fc3_usa = new_fc_layer(fc2, self.num_fc_layer2, self.num_fc_layer3_FE, use_relu = False)
    ...
    def get_detection_model(self, feature_extraction = False): 
        ...
        if feature_extraction == False: #this is the orginal lenet with 43 outputs
            ...
            self.weights_fc3_usa = new_weights(shape = [self.num_fc_layer2, self.num_fc_layer3_FE])
            self.bias_fc3_usa = new_bias(self.num_fc_layer3_FE)
            weights_conv_fc3_usa = tf.reshape(self.weights_fc3_usa, [1, 1 , self.num_fc_layer2, self.num_fc_layer3_FE ])
